package com.yj.cosmetics.util.luban;
...
public class Luban {
	...
	public static File getPhotoCacheDir(Context context, String cacheName) {
		...
		if (Log.isLoggable("Luban", Log.ERROR)) {
			LogUtils.i("default disk cache dir is null");
		}
		return null;
	}
		...
		if (gear == Luban.FIRST_GEAR)
			...
			Observable.just(data)
					...
					.doOnError(new Action1<Throwable>() {
						@Override
						public void call(Throwable throwable) {
							if (judgeCompressListener != null)
								judgeCompressListener.onError(throwable);
						}
					})
					...
					.subscribe(new Action1<JudgeGoodsData>() {
						...
						public void call(JudgeGoodsData data) {
							if (judgeCompressListener != null)
								judgeCompressListener.onJudgeSuccess(data);
						}
					});
	...
	private File saveImage(String filePath, Bitmap bitmap, long size) {
		Preconditions.checkNotNull(bitmap, "Luban" + "bitmap cannot be null");
